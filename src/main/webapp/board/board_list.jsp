...
<style type="text/css">
	...
	#header{ height: 100px; 
	background: #FFFFFF url('http://192.168.10.230/common/images/header.png') no-repeat; }
	.num{ width: 80px}
	.title{ width: 350px}
	.id{ width: 120px}
	.date{ width: 150px}
...
%>
   ...
   <%= prevMark %>... <%=pageLink %> ...<%=endMark %> --%>
	...
	int pageNumber = 3; //3개씩 보여줄 것임!
	
	//2. 화면에 보여줄 시작 페이지의 번호 설정
	int startPage = ( (currentPage-1) / pageNumber ) * pageNumber + 1;
	
	//3. 화면에 보여줄 마지막 페이지 번호 설정
	int endPage = ( ( (startPage - 1) + pageNumber ) / pageNumber ) * pageNumber;
	
	//4. 총 페이지의 수가 연산된 마지막페이지 수보다 작다면 총 페이지 수를 마지막 페이지 번호롤 설정
